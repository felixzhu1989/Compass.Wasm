...
public class IdRepository:IIdRepository
{
    ...
    {
        return _userManager.Users.FirstOrDefaultAsync(u => u.Email == email);
    }
    ...
    {
        return AddUserAsync(userName, email, "admin");
    }

    /// <summary>
    /// 创建普通用户，并指定角色
    /// </summary>
    public async Task<(IdentityResult, User?, string? password)> AddUserAsync(string userName, string email, string roleName)
    {

...
public interface ICmodfDataService : IBaseDataGetService<CmodfData>, IBaseDataUpdateService<CmodfData>
{
}

public class CmodfDataService : BaseDataGetService<CmodfData>, ICmodfDataService
{
    private readonly IDataRepository _repository;
    public CmodfDataService(IDataRepository repository) : base(repository)
    {
        _repository = repository;
    }

    public async Task<ApiResponse<CmodfData>> UpdateAsync(Guid id, CmodfData dto)
    {
        try
        {
            var model = await _repository.GetModuleDataByIdAsync(id);
            if (model is not CmodfData data) return new ApiResponse<CmodfData> { Status = false, Message = "更新数据失败" };
            ...
            return new ApiResponse<CmodfData> { Status = true, Result = dto };
        }
        catch (Exception e)
        {
            return new ApiResponse<CmodfData> { Status = false, Message = e.Message };
        }
    }
}
...
public interface IUwiDataService : IBaseDataGetService<UwiData>, IBaseDataUpdateService<UwiData>
{
}

public class UwiDataService : BaseDataGetService<UwiData>, IUwiDataService
{
    private readonly IDataRepository _repository;
    public UwiDataService(IDataRepository repository) : base(repository)
    {
        _repository = repository;
    }

    public async Task<ApiResponse<UwiData>> UpdateAsync(Guid id, UwiData dto)
    {
        try
        {
            var model = await _repository.GetModuleDataByIdAsync(id);
            if (model is not UwiData data) return new ApiResponse<UwiData> { Status = false, Message = "更新数据失败" };
            ...
            return new ApiResponse<UwiData> { Status = true, Result = dto };
        }
        catch (Exception e)
        {
            return new ApiResponse<UwiData> { Status = false, Message = e.Message };
        }
    }
}
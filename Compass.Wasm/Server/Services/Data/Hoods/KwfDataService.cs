...
public interface IKwfDataService : IBaseDataGetService<KwfData>, IBaseDataUpdateService<KwfData>
{
}

public class KwfDataService : BaseDataGetService<KwfData>, IKwfDataService
{
    private readonly IDataRepository _repository;
    public KwfDataService(IDataRepository repository) : base(repository)
    {
        _repository = repository;
    }

    public async Task<ApiResponse<KwfData>> UpdateAsync(Guid id, KwfData dto)
    {
        try
        {
            var model = await _repository.GetModuleDataByIdAsync(id);
            if (model is not KwfData data) return new ApiResponse<KwfData> { Status = false, Message = "更新数据失败" };
            ...
            return new ApiResponse<KwfData> { Status = true, Result = dto };
        }
        catch (Exception e)
        {
            return new ApiResponse<KwfData> { Status = false, Message = e.Message };
        }
    }
}
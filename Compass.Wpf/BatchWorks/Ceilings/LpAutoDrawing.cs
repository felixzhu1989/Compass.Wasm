...
public interface ILpAutoDrawing : IAutoDrawing
{

}
public class LpAutoDrawing : BaseAutoDrawing, ILpAutoDrawing
{
    #region ctor

    private readonly ILpDataService _service;
    public LpAutoDrawing(IContainerProvider provider) : base(provider)
    {
        _service = provider.Resolve<ILpDataService>();
    } 
    #endregion
    ...
    public async Task AutoDrawingAsync(ModuleDto moduleDto)
    {
        try
        {
            ...
            {
                case "LP_Z":
                    CeilingService.LpZ(swModelTop, swAssyTop, suffix, moduleDto.Name, data);
                    break;
            }

...
public interface IAnAutoDrawing : IAutoDrawing
{

}

public class AnAutoDrawing:BaseAutoDrawing,IAnAutoDrawing
{
	#region ctor

    private readonly IAnDataService _service;
    public AnAutoDrawing(IContainerProvider provider):base(provider)
    {
    ...
    public async Task AutoDrawingAsync(ModuleDto moduleDto)
    {
        try
        {
            ...
            {
                case "AN_135":
                    CeilingService.An135(swModelTop, swAssyTop, suffix, moduleDto.Name, data);
                    break;
            }

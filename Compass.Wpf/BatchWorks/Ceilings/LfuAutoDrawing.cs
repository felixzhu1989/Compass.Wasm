...
public interface ILfuAutoDrawing : IAutoDrawing
{

}

public class LfuAutoDrawing : BaseAutoDrawing, ILfuAutoDrawing
{
    #region ctor
    private readonly ILfuDataService _service;
    public LfuAutoDrawing(IContainerProvider provider) : base(provider)
    {
        _service = provider.Resolve<ILfuDataService>();
    }
    #endregion
    ...
    public async Task AutoDrawingAsync(ModuleDto moduleDto)
    {
        try
        {
            ...
            {
                case "LFU_SA":
                    CeilingService.LfuSa(swModelTop, swAssyTop, suffix, moduleDto.Name, data);
                    break;
                case "LFU_SC":
                    CeilingService.LfuSc(swModelTop, swAssyTop, suffix, moduleDto.Name, data);
                    break;
                case "LFU_SS":
                    CeilingService.LfuSs(swModelTop, swAssyTop, suffix, moduleDto.Name, data);
                    break;
            }

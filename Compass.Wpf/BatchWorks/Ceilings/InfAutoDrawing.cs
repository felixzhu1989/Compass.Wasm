...
public interface IInfAutoDrawing : IAutoDrawing
{

}

public class InfAutoDrawing:BaseAutoDrawing,IInfAutoDrawing
{
    #region MyRegion

    public readonly IInfDataService _service;
    public InfAutoDrawing(IContainerProvider provider) : base(provider)
    {
        _service = provider.Resolve<IInfDataService>();
    } 
    #endregion
    ...
    public async Task AutoDrawingAsync(ModuleDto moduleDto)
    {
        try
        {
            ...
            {
                case "INF_F":
                    CeilingService.Inf(swModelTop, swAssyTop, suffix, moduleDto.Name, data);
                    break;
            }

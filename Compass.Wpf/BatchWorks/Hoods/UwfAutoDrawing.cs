...
public interface IUwfAutoDrawing : IAutoDrawing
{
}

public class UwfAutoDrawing : BaseAutoDrawing, IUwfAutoDrawing
{
    #region ctor
    private readonly IUwfDataService _service;
    public UwfAutoDrawing(IContainerProvider provider) : base(provider)
    {
        _service = provider.Resolve<IUwfDataService>();
    }
    #endregion
    ...
    public async Task AutoDrawingAsync(ModuleDto moduleDto)
    {
        try
        {
            ...
            ExhaustService.Uw555(swAssyTop, suffix, netLength, data.SidePanel, data.UvLightType, netMiddleToRight, data.ExhaustSpigotNumber, data.ExhaustSpigotLength, data.ExhaustSpigotWidth, data.ExhaustSpigotDis, data.DrainType, data.WaterCollection, data.BackToBack, data.Marvel, data.Ansul, data.AnsulSide, data.WaterInlet);
            #endregion

            #region ExhaustSpigot_Fs，排风脖颈装配
            ExhaustService.ExhaustSpigotFs(swAssyTop, suffix, netMiddleToRight, data.ExhaustSpigotNumber, data.ExhaustSpigotLength, data.ExhaustSpigotWidth, data.ExhaustSpigotHeight, data.ExhaustSpigotDis, data.Marvel, data.Ansul, ExhaustType_e.UW);
            #endregion
            ...
            SupplyService.F555(swAssyTop, suffix, netLength, netWidth, data.Height, ExhaustType_e.UW, data.SidePanel, data.UvLightType, data.Bluetooth, data.Marvel, data.LedLogo, data.WaterCollection, data.SupplySpigotNumber, data.SupplySpigotDis);
            #endregion

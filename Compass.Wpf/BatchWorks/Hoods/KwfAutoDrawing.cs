...
public interface IKwfAutoDrawing : IAutoDrawing
{
}

public class KwfAutoDrawing : BaseAutoDrawing, IKwfAutoDrawing
{
    #region ctor
    private readonly IKwfDataService _service;
    public KwfAutoDrawing(IContainerProvider provider) : base(provider)
    {
        _service = provider.Resolve<IKwfDataService>();
    }
    #endregion
    ...
    public async Task AutoDrawingAsync(ModuleDto moduleDto)
    {
        try
        {
            ...
            {
                case "KWF_555":
                    Kwf555(data, swModelTop, swAssyTop, suffix);
                    break;
    ...
    private void Kwf555(KwfData data, ModelDoc2 swModelTop, AssemblyDoc swAssyTop, string suffix)
    {
        ...
        SupplyService.F555(swAssyTop, suffix, netLength, netWidth, data.Height, ExhaustType_e.KW, data.SidePanel, UvLightType_e.NA, false, data.Marvel, data.LedLogo, data.WaterCollection, data.SupplySpigotNumber, data.SupplySpigotDis);
        #endregion

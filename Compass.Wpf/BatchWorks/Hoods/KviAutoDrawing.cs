...
public class KviAutoDrawing : BaseAutoDrawing, IKviAutoDrawing
{
        ...
        try
        {
            ...
            switch (moduleDto.ModelName)
            {
                ...
                case "KVI_FR_555":
                    ...
                    break;
                case "KVI_FR_450":
                    KviFr450(data, swModelTop, swAssyTop, suffix);
                    break;
    ...
    private void KviFr450(KviData data, ModelDoc2 swModelTop, AssemblyDoc swAssyTop, string suffix)
    {
        #region 计算中间值与顶层操作
        //计算烟罩净长度，计算烟罩净深度
        var netLength = data.SidePanel==SidePanel_e.左||data.SidePanel==SidePanel_e.右 ? data.Length-50d : data.SidePanel==SidePanel_e.双 ? data.Length-100 : data.Length;
        ...
        SupplyService.IFr450(swAssyTop, suffix, netLength, netWidth, data.Height, ExhaustType_e.KV, data.SidePanel, UvLightType_e.NA, false, data.Marvel, data.LedLogo, data.WaterCollection, data.LightType);
        #endregion

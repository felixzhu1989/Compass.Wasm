...
{
    //注入3个实例
    private readonly IFileRepository _repository;//仓储
    ...
    public async Task<UploadedItem> UploadAsync(Stream stream, string fileName, CancellationToken cancellationToken)
    {
        ...
        //fileName是原始文件名
        string key = $"{today.Year}/{today.Month}/{hash}/{fileName}";
        ...
        {
            oldUploadItem.IsOldFile = true;
            return oldUploadItem;//如果存在则直接返回旧文件
        }
        ...
        //return UploadedItem.Create(id, fileSize, fileName, hash, new Uri("http://localhost:5070/swagger/index.html"), remoteUrl);
    }
}